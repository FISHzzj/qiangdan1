...
if (!(class_exists('CommissionModel'))) {
    class CommissionModel extends PluginModel
    {
        ...
        public function calculate($orderid = 0, $update = true)
        {
            ...
            $hascommission = false;
            $goods = pdo_fetchall('select og.id,og.realprice,og.total,g.hasoption,og.goodsid,og.optionid,g.money,g.hascommission,g.nocommission, g.commission1_rate,g.commission1_pay,g.commission2_rate,g.commission2_pay,' . "\r\n" . '                  g.commission3_rate,g.commission3_pay,g.commission,og.commissions,og.seckill,og.seckill_taskid,og.seckill_timeid from ' . tablename('wx_shop_order_goods') . '  og ' . ' left join ' . tablename('wx_shop_goods') . ' g on g.id = og.goodsid' . ' where og.orderid=:orderid and og.uniacid=:uniacid', array(':orderid' => $orderid, ':uniacid' => $_W['uniacid']));
            if (0 < $set['level']) {
                foreach ($goods as &$cinfo) {
                    if($cinfo['money'] > 0){

                        if (strexists($cinfo['money'], '%')){
                            $price = $cinfo['realprice']*floatval(str_replace('%', '', $cinfo['money'])) / 100 * $rate;
                        }else{
                            $price = $cinfo['realprice']*$cinfo['money'] * $rate;
                        }
                    }else{
                    ...
                    if ($update) {
                        ...
                        if (!(empty($agentid))) {
                            ...
                            if (($m1['isagent'] == 1) && ($m1['status'] == 1)) {
                                ...
                                if( pdo_insert('wx_shop_member_commission',array('uniacid'=>$_W['uniacid'],'openid'=>$m1['openid'],'orderid'=>$orderid,'level'=>1,'goodsid'=>$cinfo['id'],'money'=>$commissions['level1'],'createtime'=>time()))){
                                    $this->addreward($commissions['level1'],$m1['openid']); 
                                }
                                if (!(empty($m1['agentid']))) {
                                    ...
                                    if( pdo_insert('wx_shop_member_commission',array('uniacid'=>$_W['uniacid'],'openid'=>$m2['openid'],'orderid'=>$orderid,'level'=>2,'goodsid'=>$cinfo['id'],'money'=>$commissions['level2'],'createtime'=>time()))){
                                        $this->addreward($commissions['level2'],$m2['openid']); 
                                    }
                                    if (!(empty($m2['agentid']))) {
                                        ...
                                        if(pdo_insert('wx_shop_member_commission',array('uniacid'=>$_W['uniacid'],'openid'=>$m3['openid'],'orderid'=>$orderid,'level'=>3,'goodsid'=>$cinfo['id'],'money'=>$commissions['level3'],'createtime'=>time()))){
                                            $this->addreward($commissions['level3'],$m3['openid']); 
                                        }
        ...
        public function getInfo($openid, $options = NULL, $start='',$end = '')
        {
            ...
            if (1 <= $level) {
                ...
                if (in_array('ok', $options)) {
                    if($start and $end){
                        $level1_commissions = pdo_fetchall('select og.commission1,og.commissions  from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join  ' . tablename('wx_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid=:agentid and o.status>=3 and o.issue = 1 and og.nocommission=0 and (' . $time . ' - o.finishtime > ' . $day_times . ') and og.status1=0  and o.uniacid=:uniacid and o.isparent=0 and o.createtime between '.$start.' and '.$end.' ', array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id'],));
                    }else{
                         $level1_commissions = pdo_fetchall('select og.commission1,og.commissions  from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join  ' . tablename('wx_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid=:agentid and o.status>=3 and o.issue = 1 and og.nocommission=0 and (' . $time . ' - o.finishtime > ' . $day_times . ') and og.status1=0  and o.uniacid=:uniacid and o.isparent=0', array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']));
                    }
            ...
            if (2 <= $level) {
                if (0 < $level1) {
                    ...
                    if (in_array('ok', $options)) {
                        if($start and $end){
                            $level2_commissions = pdo_fetchall('select og.commission2,og.commissions  from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join  ' . tablename('wx_shop_order') . ' o on o.id = og.orderid ' . ' where o.agentid in( ' . implode(',', array_keys($level1_agentids)) . ')  and (' . $time . ' - o.finishtime > ' . $day_times . ') and o.status>=3 and o.issue = 1 and og.status2=0 and og.nocommission=0  and o.uniacid=:uniacid and o.isparent=0 and o.createtime between '.$start.' and '.$end.' ', array(':uniacid' => $_W['uniacid']));
                        }else{
                            $level2_commissions = pdo_fetchall('select og.commission2,og.commissions  from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join  ' . tablename('wx_shop_order') . ' o on o.id = og.orderid ' . ' where o.agentid in( ' . implode(',', array_keys($level1_agentids)) . ')  and (' . $time . ' - o.finishtime > ' . $day_times . ') and o.status>=3 and o.issue = 1 and og.status2=0 and og.nocommission=0  and o.uniacid=:uniacid and o.isparent=0', array(':uniacid' => $_W['uniacid']));
                        }
            ...
            if (3 <= $level) {
                if (0 < $level2) {
                    ...
                    if (in_array('ok', $options)) {
                        if($start and $end){
                            $level3_commissions = pdo_fetchall('select og.commission3,og.commissions  from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join  ' . tablename('wx_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid in( ' . implode(',', array_keys($level2_agentids)) . ')  and (' . $time . ' - o.finishtime > ' . $day_times . ') and o.status>=3 and o.issue = 1 and og.status3=0  and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0 and o.createtime between '.$start.' and '.$end.' ', array(':uniacid' => $_W['uniacid']));
                        }else{
                            $level3_commissions = pdo_fetchall('select og.commission3,og.commissions  from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join  ' . tablename('wx_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid in( ' . implode(',', array_keys($level2_agentids)) . ')  and (' . $time . ' - o.finishtime > ' . $day_times . ') and o.status>=3 and o.issue = 1 and og.status3=0  and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0', array(':uniacid' => $_W['uniacid']));
                        }   
            ...
            $this->getInfo = $member;
            if($start and $end){
                return $member['commission_pay']+$member['commission_ok'];
            }else{
                return $this->getInfo;
            }
        ...
        public function createMyShopQrcode($mid = 0, $posterid = 0)
        {
            ...
            if (!(is_dir($path))) {
              ...
              $url = mobileUrl('account/register', array('mid' => $mid), true);
			// $url = $_W['siteroot'].'dist/index.html#/login?mid='.$mid;
            if (!(empty($posterid))) {
        ...
        public function createShopImage($openid = '')
        {
            global $_W;
            global $_GPC;
        ...
        public function checkAgent($openid = '',$mids='')
        {
            global $_W;
            global $_GPC;
            $set = $this->getSet();
            ...
            if (!(empty($mid))) {
                ...
                $parent = m('member')->getMember($mids);
            }
        ...
        public function addreward($money,$openid)
        {
            global $_W;
            $date = strtotime(date("Y-m-d", time()));
            //查询今天是否有记录
            $hastoday = pdo_fetch("select id,money from " .tablename('wx_shop_member_commission1'). "where createtime = :createtime and openid = :openid", array('createtime'=>$date, 'openid'=>$openid));
            if ($hastoday) {
                $data['money'] = $hastoday['money'] + $money;
                pdo_update('wx_shop_member_commission1', $data, array('openid'=>$hastoday['id']));
            }else{
                $data1['uniacid'] = $_W['uniacid'];
                $data1['money'] = $money;
                $data1['openid'] = $openid;
                $data1['createtime'] = $date;
                pdo_insert('wx_shop_member_commission1', $data1);
            }
        }

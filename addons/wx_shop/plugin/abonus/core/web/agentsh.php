...
class Agentsh_WxShopPage extends PluginWebPage 
{
	public function main() 
	{
		...
		$sql = 'select dm.*,dm.nickname,dm.avatar,l.levelname,p.nickname as parentname,p.avatar as parentavatar,dm.aagentprovinces,dm.aagentcitys from ' . tablename('wx_shop_member') . ' dm ' . ' left join ' . tablename('wx_shop_member') . ' p on p.id = dm.agentid ' . ' left join ' . tablename('wx_shop_abonus_level') . ' l on l.id = dm.aagentlevel' . ' where dm.uniacid = ' . $_W['uniacid'] . ' and dm.isaagent =1 AND dm.aagentstatus =0 ' . $condition . ' ORDER BY dm.aagenttime desc';
		...
		foreach ($list as &$row ) 
		{
			...
			if($row['aagentprovinces']){

				$aagentprovinces = unserialize($row['aagentprovinces']);

				$row['dlcs'] = $aagentprovinces['0'];
			}else if($row['aagentcitys']){
				
				$aagentcitys = unserialize($row['aagentcitys']);

				$row['dlcs'] = $aagentcitys['0'];
	...
	public function delete() 
	{
		...
		{
			pdo_update('wx_shop_member', array('isaagent' => 0, 'aagentstatus' => 0,'dltype'=>'0'), array('id' => $member['id']));
			//押金退回
			$deposit = $member['deposit'];

			pdo_update('wx_shop_member',['credit2'=>$member['credit2']+=$deposit,'deposit'=>$member['deposit']-=$deposit,'credit3'=>$member['credit3']-=$deposit,'dltype'=>'0'],['uniacid'=>$_W['uniacid'],'id'=>$member['id']]);


			//记录
			pdo_insert('wx_shop_member_credit_log',array('uniacid'=>$_W['uniacid'],'openid'=>$member['openid'],'orderid'=>0,'money'=>$deposit,'type'=>7,'createtime'=>time()));
			//记录
			pdo_insert('wx_shop_member_credit_log',array('uniacid'=>$_W['uniacid'],'openid'=>$member['openid'],'orderid'=>0,'money'=>-$deposit,'type'=>6,'createtime'=>time()));
			pdo_update('7033_yjjiedong',['status'=>'2','uptime'=>time(),'tjtime'=>time()],['uniacid'=>$_W['uniacid'],'uid'=>$member['id'],'type'=>$member['dltype']]);
	...
	public function check() 
	{
		...
		$members = pdo_fetchall('SELECT id,openid,nickname,realname,mobile,aagentstatus,dlyjtype FROM ' . tablename('wx_shop_member') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
		$time = time();
		foreach ($members as $member ) 
		{
			...
			if ($status == 1) 
			{
				...
				pdo_update('wx_shop_member', array('aagentstatus' => 1, 'aagenttime' => $time,'dlyjtype'=>'1'), array('id' => $member['id'], 'uniacid' => $_W['uniacid']));
			
				//开通代理直推奖励
				$this->agentreward($member['id']);
	...
	//下级成为代理，直推拿奖励
	public function agentreward($uid){
		global $_W;
		global $_GPC;

		$member = pdo_fetch('SELECT id,openid,agentid,deposit,dltype,dlyjtype FROM'.tablename('wx_shop_member').'WHERE uniacid=:uniacid AND id=:id ',[':uniacid'=>$_W['uniacid'],':id'=>$uid]);

		if($member['agentid'] && $member['dlyjtype']==0){

			$set = $_W['shopset']['abonus'];

			$reward = $member['deposit']*($set['zhitui']/100);

			$agent = pdo_fetch('SELECT openid FROM'.tablename('wx_shop_member').'WHERE uniacid=:uniacid AND id=:id ',[':uniacid'=>$_W['uniacid'],'id'=>$member['agentid']]);
			//扣除余额为押金
			m('member')->setCredit($agent['openid'], 'credit2', $reward, array($_W['uid'], '申请代理扣除余额为押金' .'余额'));
			//记录
			pdo_insert('wx_shop_member_credit_log',array('uniacid'=>$_W['uniacid'],'openid'=>$agent['openid'],'orderid'=>0,'money'=>$reward,'type'=>8,'createtime'=>time()));

			$deposit = $member['deposit'] - $reward;

			pdo_update('wx_shop_member',['credit3'=>$deposit,'deposit'=>$deposit],['uniacid'=>$_W['uniacid'],'id'=>$member['id']]);
			
			pdo_update('7033_yjjiedong',['money'=>$deposit],['uniacid'=>$_W['uniacid'],'uid'=>$member['id'],'type'=>$member['dltype']]);
			//记录
			pdo_insert('wx_shop_member_credit_log',array('uniacid'=>$_W['uniacid'],'openid'=>$member['openid'],'orderid'=>0,'money'=>$deposit,'type'=>9,'createtime'=>time()));

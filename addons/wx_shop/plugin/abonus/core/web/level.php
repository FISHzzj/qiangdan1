...
class Level_WxShopPage extends PluginWebPage 
{
	public function main() 
	{
        ...
        $default = array('id' => 'default', 'levelname' => empty($set['levelname']) ? '默认等级' : $set['levelname'],'weight'=> empty($set['weight']) ? 1 : $set['weight'], 'bonus1' => $set['bonus1'], 'bonus2' => $set['bonus2'], 'bonus3' => $set['bonus3'],'zhitui'=>$set['zhitui']);
	...
	protected function post() 
	{
		...
		$id = trim($_GPC['id']);
		if ($id == 'default') {
            $level = array('id' => 'default', 'levelname' => empty($set['levelname']) ? '默认等级' : $set['levelname'], 'weight'=> empty($set['weight']) ? 1 : $set['weight'],  'bonus1' => $set['bonus1'], 'bonus2' => $set['bonus2'], 'bonus3' => $set['bonus3'],'zhitui'=>$set['zhitui']);
		} else {

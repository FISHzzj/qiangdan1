...
class List_WxShopPage extends WebPage
{
	public function main() 
	{
		...
		if (!(empty($list_level))) 
		{
			...
			$res_level = pdo_fetchall('select id,levelname from ' . tablename('wx_shop_sd_member_level') . ' where id in (' . implode(',', $list_level) . ')', array(), 'id');
		}
		...
		$levels = pdo_fetchall(' select level, levelname from ' . tablename('wx_shop_sd_member_level') . ' where uniacid = :uniacid order by level asc ', array(':uniacid' => $_W['uniacid']));
...
//		    exit();
			...
			if ($openbind) 
			{
				...
				if (!(empty($data['pwd2']))) 
				{
					...
					$data['pwd2'] = md5($data['pwd2'] . $salt);
					$data['salt'] = $salt;
				}
				else 
				{
					unset($data['pwd2']);
					unset($data['pwd2'], $data['salt']);
				}
			}
			...
			if ($hasabonus) 
			{
				if (cv('abonus.agent.check')) 
				{
					...
					if (!(empty($aadata))) 
					{
						...
						if(count($aagentprovinces)>1){

							show_json(0,'一个会员只能设置一个代理');

						}
						//市区代理参数
						$aagentcitys = ((is_array($_GPC['aagentcitys']) ? $_GPC['aagentcitys'] : array()));

						if(count($aagentcitys)>1){

							show_json(0,'一个会员只能设置一个代理');

						}
						$setdl = pdo_fetch('SELECT aagentprovinces,aagentcitys FROM'.tablename('wx_shop_member').'WHERE uniacid=:uniacid AND id=:id ',[':uniacid'=>$_W['uniacid'],':id'=>$id]);
						// var_dump($_GPC);exit;
						if(iunserializer($setdl['aagentcitys']) && $aagentcitys && !$_GPC['data']){

							show_json(0,'该会员已经是代理，如果需要更改请先取消原来的设置');
						}

						if(iunserializer($setdl['aagentprovinces'])&& $aagentprovinces && !$_GPC['data']){

							show_json(0,'该会员已经是代理，如果需要更改请先取消原来的设置');
						}
						// // var_dump($aagentcitys);die;
						//查所有审核通过代理商
						...
						foreach ($dls as $k => $v) {
							...
							if($aagentprovinces){
								...
								if($aagentprovinces == $provinces && $v['id'] != $id){
	...
	public function setblack() 
	{
		...
		foreach ($members as $member ) 
		{
			...
			{
				pdo_update('wx_shop_member', array('isblack' => 0, 'updatePwd' => 0), array('id' => $member['id']));
		...
		if ($_W['ispost']) 
		{
			...
			else 
			{
				if (!(empty($levelid))) 
				{
					...
					$level = pdo_fetch('select * from ' . tablename('wx_shop_sd_member_level') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $levelid, ':uniacid' => $_W['uniacid']));
					if (empty($level)) 
					{
						show_json(0, '此等级不存在');
					}

...
class Level_WxShopPage extends WebPage
{
	public function main() 
	{
		...
		$others = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_member_fanyong') . ' WHERE 1 ' . $condition . ' ORDER BY level asc', $params);
	...
	protected function post() 
	{
		...
		{
			$level = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_member_fanyong') . ' WHERE id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => intval($id)));
			if (!(empty($level))) 
			{
		...
		if ($_W['ispost']) 
		{
			...
			if (!(empty($id))) 
			{
				...
				else 
				{
					...
					pdo_update('wx_shop_member_fanyong', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
			...
			else 
			{
				$data['goodsids'] = $goodsids;
				$data['buygoods'] = $buygoods;
				pdo_insert('wx_shop_member_fanyong', $data);
				$id = pdo_insertid();
	...
	public function delete() 
	{
		...
		$items = pdo_fetchall('SELECT id,levelname FROM ' . tablename('wx_shop_member_fanyong') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
		foreach ($items as $item ) 
		{
			pdo_delete('wx_shop_member_fanyong', array('id' => $item['id']));
	...
	public function enabled() 
	{
		...
		foreach ($items as $item ) 
		{
			pdo_update('wx_shop_member_fanyong', array('enabled' => intval($_GPC['enabled'])), array('id' => $item['id']));

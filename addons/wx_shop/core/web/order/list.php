...
class List_WxShopPage extends WebPage
{
	protected function orderData($status, $st) 
	{
		...
		if($_GPC['freeze'] != ''){
			$condition .= ' AND o.freeze =' . intval($_GPC['freeze']);
		}
		if($_GPC['is_submit'] != ''){
			$condition .= ' AND o.is_submit =' . intval($_GPC['is_submit']);
		}
		if($_GPC['issue'] != ''){
			$condition .= ' AND o.issue =' . intval($_GPC['issue']);
		}
	...
	public function newList()
	{
		...
		$sql = ' select o.id, o.ordersn, o.price, o.commission, o.status, o.createtime, o.freeze, o.halllevel, m.nickname, m.mobile, m.realname, g.title, g.thumb from ' . tablename('wx_shop_sd_order') . ' o ' . ' left join ' . tablename('wx_shop_member') . ' m on m.id = o.uid ' . ' left join ' . tablename('wx_shop_goods') . ' g on g.id = o.goodsid ' . ' where ' . $condition . ' order by o.createtime asc ';
		$sql .= ' limit ' . ($pindex - 1) * $psize . ',' . $psize;
		...
		$total = pdo_fetchcolumn(' select count(o.id) from ' . tablename('wx_shop_sd_order') . ' o ' . ' left join ' . tablename('wx_shop_member') . ' m on m.id = o.uid ' . ' left join ' . tablename('wx_shop_goods') . ' g on g.id = o.goodsid ' . ' where ' . $condition, $params);
		$pager = pagination2($total, $pindex, $psize);
		include $this->template('order/list');
	}
}

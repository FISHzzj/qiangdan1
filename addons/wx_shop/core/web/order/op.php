...
class Op_WxShopPage extends WebPage
{
	...
	public function jdorder(){
		...
		$order = pdo_fetch('SELECT id,price,canceltime,createtime,openid FROM'. tablename('wx_shop_order').' where freeze = 1 and id = :id and uniacid = :uniacid and refund = 0 ',array(':id'=>$orderid,':uniacid'=>$_W['uniacid']));
		// var_dump($order);die;
		if(!$order){

			show_json(0,'没有查到该冻结订单');
		}

		pdo_update('wx_shop_member',array('credit2 +='=>$order['price']),array('openid'=>$order['openid']));

...
{   
    ...
    public function main()
    {
        ...
        $cumulative = pdo_fetchcolumn(' select sum(interest) from ' . tablename('wx_shop_sd_transactions_list') . ' where uniacid = :uniacid and uid = :uid and status = 3 ', array(':uniacid' => $uniacid, ':uid' => $this->member['id']));
        $totalMoney = pdo_fetchcolumn(' select sum(money) from ' . tablename('wx_shop_sd_transactions_list') . ' where uniacid = :uniacid and uid = :uid and status = 3 ', array(':uniacid' => $uniacid, ':uid' => $this->member['id']));
        ...
        $info = array(
            ...
            'cumulativeEarningRate'     => ($totalMoney == 0 || $cumulative == 0) ? 0 : round($cumulative / $totalMoney * 100, 2),//累计收益率
    ...
    public function extract()
    {
        ...
        $info = pdo_fetch(' select * from ' . tablename('wx_shop_sd_transactions_list') . ' where uniacid = :uniacid and id = :id and uid = :uid and status = 0 ', array(':uniacid' => $uniacid, ':uid' => $this->member['id'], ':id' => $id));
        if(empty($info)){
            show_json(0, '该理财已提取');
        }
        ...
        if($op == 'sub'){
            ...
            if($money > 0){
                ...
                if(flock($fp,LOCK_EX | LOCK_NB)){
                    ...
                    if($result){
                        pdo_update('wx_shop_sd_transactions_list', array('status' => 2, 'updatetime' => time()), array('id' => $info['id']));
                        show_json(1, '提取成功');
                    }else{
                        show_json(0, '系统繁忙，请稍后再试');
                    }

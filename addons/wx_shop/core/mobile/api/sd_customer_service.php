...
{   
    ...
    { 
        ...
        if(empty($levelId)){
            ...
            $levelInfo = pdo_fetch(' select level, levelname, hallname, c_proportion, recharge, invitations_num from ' . tablename('wx_shop_sd_member_level') . ' where uniacid = :uniacid and level = :level ', array(':uniacid' => $uniacid, ':level' => $levelId));
        }
        ...
        // if($this->member['level'] < $levelInfo['level']){
            //推荐人数和充值金额(提示还需N人开启大厅)
                ...
                foreach ($InvitationNum as $key => $value) {
                    if($value['recharge'] >= $levelInfo['recharge']){//个人累计充值满足升级条件
                ...
                $difference = intval($levelInfo['invitations_num'] - $newInvitationCount);
                if($difference > 0){
                    show_json(0, '距离下一级还需要邀请'.$difference.'人');
                }
        //     show_json(0, '该大厅还未解锁');
        // }
        ...
        $tasknum = pdo_fetchcolumn(' select count(id) from ' . tablename('wx_shop_sd_order') . ' where uniacid = :uniacid and uid = :uid ', array(':uniacid' => $uniacid, ':uid' => $this->member['id']));
        $taskprice = pdo_fetchcolumn(' select sum(commission) from ' . tablename('wx_shop_sd_order') . ' where uniacid = :uniacid and uid = :uid and status = 2 ', array(':uniacid' => $uniacid, ':uid' => $this->member['id']));
        $frozen = pdo_fetchcolumn(' select sum(price) from ' . tablename('wx_shop_sd_order') . ' where uniacid = :uniacid and uid = :uid and freeze = 1 ', array(':uniacid' => $uniacid, ':uid' => $this->member['id']));

        $info = array(
            'tasknum'   => empty($tasknum) ? 0 : $tasknum,
            'taskprice' => empty($taskprice) ? 0 : $taskprice,
            'frozen'    => empty($frozen) ? 0 : $frozen,
    ...
    protected function grabSheet($levelInfo)
    {
        ...
        $hangUpNum = pdo_fetchcolumn(' select count(id) from ' . tablename('wx_shop_sd_order') . ' where uniacid = :uniacid and uid = :uid and status = 1 and freeze = 0 ', array(':uniacid' => $uniacid, ':uid' => $this->member['id']));//当前挂单数量（未提交订单数量）
        if(empty($hangUpNum)){
            $hangUpNum = 0;
        }
    ...
    protected function grabSheetStart($taskSet, $levelInfo)
    {
        ...
        if(flock($fp,LOCK_EX | LOCK_NB)){
            ...
            $sd_model = m('sd_model');
            $payResult = $sd_model->setGold($this->member['id'], $this->member['openid'], 'credit2', -$orderMoney, 1, $orderid, 1);
            if(!$payResult){
                pdo_update('wx_shop_sd_order', array('status' => -1), array('id' => $orderid));//付款失败
                flock($fp,LOCK_UN);
                fclose($fp);
                show_json(0, '扣款失败');
            }
    ...
    public function subOrder()
    {
        ...
        if(flock($fp,LOCK_EX | LOCK_NB)){
            ...
            if($result1 && $result2){
                ...
                //分润
                $sd_model->distribution($order['id']);

...
class Member_WxShopModel
{
	...
	// 查找下级
	public function getmylower_threes($agentid = '', &$arr,$i=0,$ceng = 3 ){
        ...
        $sql = "SELECT id,openid,nickname,mobile,credit2,createtime,avatar from " . tablename('wx_shop_member') . " as m where agentid in(".$agentid.")";
        ...
        foreach ($getmyl as $k => &$v) {
        	...
        	$countzt = pdo_fetchcolumn('SELECT count(id) FROM'.tablename('wx_shop_member').'WHERE uniacid=:uniacid AND agentid=:agentid ',[':uniacid'=>$_W['uniacid'],':agentid'=>$v['id']]);
        	$v['countzt'] = $countzt;
        ...
        if($cc){
           $this->getmylower_threes($cc, $arr,$i);
        }
        return $arr;
    }

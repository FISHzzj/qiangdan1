...
//        $package['openid'] =str_replace('sns_wa_','',$_GPC['openid']);
    ...
    public function deleteFile($attachment, $fileDelete = false)
    {
        ...
        /** 生成随机数
     * @param $randLength   长度
     * @param $addtime  是否加上时间
     * @param $includenumber 是否加上数字
     * @param $table 表
     * @param $field 字段
     * @return string
     */
    public function rand_str($randLength = 6, $addtime = 1, $includenumber = 0,$table='wx_shop_member',$field = 'rcode')   //生成随机数
	{
		if ($includenumber) {
        ...
        //     $res = pdo_fetch('select id from ' . tablename($table) . 'where '.$field.' = :field',array(':feild'=>$tokenvalue));
        //     if($res){
        //         $this->rand_str($randLength, $addtime, $includenumber,$table,$field);
        //     }
        // }
		return $tokenvalue;
	}

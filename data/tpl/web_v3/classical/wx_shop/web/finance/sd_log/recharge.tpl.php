...
<div class="page-content">
    <form action="./index.php" method="get" class="form-horizontal table-search" role="form" id="form1">
        ...
        <input type="hidden" name="r" value="finance.sd_log.recharge" />
                    ...
                    <tbody>
                        <?php  if(is_array($list)) { foreach($list as $row) { ?>
                            <tr>
                                ...
                                <td><img src="<?php  echo $row['images'];?>" width="200px"></td>
                                ...
                                <td>
                                    <?php  if($row['status'] == 0) { ?>
                                        <a class='btn btn-sm btn-primary' data-toggle='ajaxPost' data-confirm="确认通过?" href="<?php  echo webUrl('finance/sd_log/rechargeExamine',array('id' => $row['id'], 'status' => 1));?>">
                                            通过
                                        </a>
                                        <a class='btn btn-sm btn-danger' data-toggle='ajaxPost' data-confirm="确认驳回?" href="<?php  echo webUrl('finance/sd_log/rechargeExamine',array('id' => $row['id'], 'status' => 2));?>">
                                            驳回
                                        </a>
                                    <?php  } ?>
                                </td>

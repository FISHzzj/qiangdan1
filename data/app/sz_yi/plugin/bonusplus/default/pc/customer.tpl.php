...
.order_menu {height:44px; background:#f8f8f8;padding: 10px;}
...
.order_menu .user {font-size: 18px;}

.order_menu .user span{float:right;}
...
.order_list .left .inner { width:100%;margin-right:-20%;padding-left: 10px;margin-left: 50px;margin-top:-40px;}
...
.order_detail {overflow:hidden; padding:10px; background:#fff; border-top:1px solid #eaeaea;}
   ...
   .team_list {height:40px; background:#fff; }
    ...
    .rightlist{width: 960px;float: left;margin-left: 15px;}
</style>
...
<div style="width: 960px;float: left;margin-left: 15px;">
    ...
    <div class="order_top">
        ...
        <div class="title" onclick='history.back()'><?php  echo $this->set['texts']['mycustomer']?>(<?php  echo $member['customercount1'];?>)</div>

    </div>

    <div class="order_menu">

            <div class="user">会员信息<span>TA的消费订单/金额</span></div>
...
<script id='tpl_customer' type='text/html'>
  ...
  <div class="order_list">
    ...
    <div class="left"><img src="<%if user.nickname%><%user.avatar%><%else%>../addons/sz_yi/plugin/commission/images/head.jpg<%/if%>" style="width:40px;margin-left:10px;"><div class='inner'><%if user.nickname%><%user.nickname%><%else%>未获取<%/if%></br>注册时间: <%user.createtime%></div></div>

   <div class="right"><%user.ordercount%> 订单</br><%user.moneycount%> 元</div>
...
function bindScroller(){
        ...
        $(window).scroll(function () {
            ...
            if ($(document).height() <= totalheight) {
                ...
                if (stop == true) {
                    ...
                    core.pjson('bonusplus/customer', {status:current_status,page: page}, function (morejson) {
        ...
        function getOrder(status) {
            ...
            core.pjson('bonusplus/customer', {status:status, page: page}, function (json) {
